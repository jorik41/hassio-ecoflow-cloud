"""Delta Pro Ultra public API device support."""
...
class DeltaProUltra(BaseDevice):
    """Device description for Delta Pro Ultra."""
    ...
    def sensors(self, client: EcoflowApiClient) -> list[BaseSensorEntity]:
        return [
            ...
            MiscSensorEntity(
                client, self, "hs_yj751_pd_appshow_addr.bpNum", const.BATTERY_COUNT
            ),
            MiscSensorEntity(
                client,
                self,
                "hs_yj751_pd_appshow_addr.fullCombo",
                const.WIRELESS_4G_DATA_MAX,
            ),
            MiscSensorEntity(
                client,
                self,
                "hs_yj751_pd_appshow_addr.remainCombo",
                const.WIRELESS_4G_DATA_REMAINING,
            ),
            MiscSensorEntity(
                client,
                self,
                "hs_yj751_pd_appshow_addr.wireless4gCon",
                const.WIRELESS_4G_REGISTERED,
            ),
            MiscSensorEntity(
                client,
                self,
                "hs_yj751_pd_appshow_addr.wirlesss4gErrCode",
                const.WIRELESS_4G_ERROR_CODE,
            ),
            MiscSensorEntity(
                client,
                self,
                "hs_yj751_pd_appshow_addr.simIccid",
                const.WIRELESS_4G_SIM_ID,
            ),
            MiscSensorEntity(
                client,
                self,
                "hs_yj751_pd_appshow_addr.wireless4GSta",
                const.INTERNET_CONNECTION_TYPE,
            ),
            MiscSensorEntity(
                client,
                self,
            ...
            OutWattsSensorEntity(
                ...
                const.PIO_PORT_OUT_POWER,
            ),
            AmpSensorEntity(
                client,
                self,
                "hs_yj751_pd_backend_addr.outAc5p8Amp",
                const.PIO_PORT_OUT_CURRENT,
                False,
            ),
            VoltSensorEntity(
                client,
            ...
            OutWattsSensorEntity(
                ...
                const.AC_N_OUT_POWER % 1,
            ),
            AmpSensorEntity(
                client,
                self,
                "hs_yj751_pd_backend_addr.outAcL11Amp",
                const.AC_N_OUT_CURRENT % 1,
                False,
            ),
            VoltSensorEntity(
                client,
                self,
                "hs_yj751_pd_backend_addr.outAcL11Vol",
                const.AC_N_OUT_VOLTAGE % 1,
                False,
            ),
            FrequencySensorEntity(
                client,
            ...
            OutWattsSensorEntity(
                ...
                const.AC_N_OUT_POWER % 2,
            ),
            AmpSensorEntity(
                client,
                self,
                "hs_yj751_pd_backend_addr.outAcL12Amp",
                const.AC_N_OUT_CURRENT % 2,
                False,
            ),
            VoltSensorEntity(
                client,
                self,
                "hs_yj751_pd_backend_addr.outAcL12Vol",
                const.AC_N_OUT_VOLTAGE % 2,
                False,
            ),
            FrequencySensorEntity(
                client,
            ...
            OutWattsSensorEntity(
                ...
                const.AC_N_OUT_POWER % 3,
            ),
            AmpSensorEntity(
                client,
                self,
                "hs_yj751_pd_backend_addr.outAcL21Amp",
                const.AC_N_OUT_CURRENT % 3,
                False,
            ),
            VoltSensorEntity(
                client,
            ...
            OutWattsSensorEntity(
                ...
                const.AC_N_OUT_POWER % 4,
            ),
            AmpSensorEntity(
                client,
                self,
                "hs_yj751_pd_backend_addr.outAcL22Amp",
                const.AC_N_OUT_CURRENT % 4,
                False,
            ),
            VoltSensorEntity(
                client,
            ...
            OutWattsSensorEntity(
                ...
                const.AC_N_OUT_POWER % 5,
            ),
            AmpSensorEntity(
                client,
                self,
                "hs_yj751_pd_backend_addr.outAcL14Amp",
                const.AC_N_OUT_CURRENT % 5,
                False,
            ),
            VoltSensorEntity(
                client,
            ...
            OutWattsSensorEntity(
                ...
                const.AC_N_OUT_POWER % 6,
            ),
            AmpSensorEntity(
                client,
                self,
                "hs_yj751_pd_backend_addr.outAcTtAmp",
                const.AC_N_OUT_CURRENT % 6,
                False,
            ),
            VoltSensorEntity(
                client,
    ...
    def _prepare_data(self, raw_data) -> dict[str, any]:
        ...
        if (
            "hs_yj751_pd_appshow_addr.showFlag" in res["params"]
            and isinstance(res["params"]["hs_yj751_pd_appshow_addr.showFlag"], int)
        ):
            documentation_bit_order = (
                (res["params"]["hs_yj751_pd_appshow_addr.showFlag"] >> 4) & 3855
            ) | (res["params"]["hs_yj751_pd_appshow_addr.showFlag"] << 4)
            for offset in range(16):
                res["params"][
                    f"hs_yj751_pd_appshow_addr.showFlag.{offset + 1}"
                ] = (documentation_bit_order >> offset) & 1
        return res

    def to_plain_nested_addr_prefix(self, raw_data: dict[str, any]) -> dict[str, any]:
        if "typeCode" in raw_data:
            prefix = data_bridge.status_to_plain.get(
        ...
        for key, value in raw_data.items():
            if key not in {"param", "params"}:
                result[key] = value
        return result

    def nested_to_top_level(self, dest: dict[str, any], key: str, value: any) -> None:
        if isinstance(value, dict):
            for nested_key, nested_value in value.items():
                self.nested_to_top_level(dest, f"{key}.{nested_key}", nested_value)
        elif isinstance(value, list):
            for idx, nested_value in enumerate(value):
                self.nested_to_top_level(dest, f"{key}.{idx}", nested_value)
        else:
            dest[key] = value
